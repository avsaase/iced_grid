use iced::{
    ...
    event, mouse, overlay, Element, Event, Length, Pixels, Point, Rectangle, Size,
};
...
/// A container that distributes its contents in a grid of rows and columns. The number of columns
/// is determined by the row with the most elements.
    ...
    Renderer: iced_widget::core::Renderer,
{
    ...
    fn layout(&self, renderer: &Renderer, limits: &Limits) -> Node {
        ...
        // Calculate the minimum column widths and row heights to fit the contents
        ...
        let max_row_height = *min_row_heights
            .iter()
            .max_by(|a, b| a.partial_cmp(b).unwrap())
            .unwrap();
        let max_col_width = *min_columns_widths
            .iter()
            .max_by(|a, b| a.partial_cmp(b).unwrap())
            .unwrap();
